begin tran t1
	DECLARE @copyDate varchar(10) = CAST('20171020' AS DATE)	
	DECLARE @st_targetDate varchar(8) = '20171107'
	DECLARE @ed_targetDate varchar(8) = '20171110'
	
	DECLARE @st_dayDiff as  int = DATEDIFF(day, @copyDate ,@st_targetDate)
	DECLARE @et_dayDiff as  int = DATEDIFF(day, @st_targetDate ,@ed_targetDate)
	DECLARE @tmp_Diff as  int = 0
    	DECLARE @dayDiff as int = 0
	
	while (@tmp_Diff <= @et_dayDiff)
		begin
		SET @dayDiff = @st_dayDiff + @tmp_Diff
		SET @tmp_Diff = @tmp_Diff + 1
		DECLARE @targetDate varchar(8) =  convert(varchar(8), dateadd(day, @tmp_Diff, @st_targetDate), 112)
		...
		select *, m_hdrec_id = substring(hdrec_id, 1, 32 - 8) + @targetDate
		 ...
		 into #temp_master
		  ...
		  FROM (
			...
			where hd_date = @copyDate AND A.PAT_NO in ('0000922786', '0000943018', '0001002676', '0001052546', '0001117267', '0001147256', '0001908465', '0001573098', '0001618550', '0001238389', '0001539877', '0001507214', '0001825357', '0001504697', '0000797718')
